...
export default function AllArt() {
    ...
    <>
      ...
      <div className="container mx-auto mt-8 px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {art.length > 0 ? (
            art.map((item) => (
              <div key={item._id} style={{ padding: "10px" }} className="bg-white shadow-lg rounded-lg overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  <div className="border-t mt-3 pt-3 flex justify-around items-center text-gray-500">
                    <button style={{ border: "none", background: "none" }} className="flex items-center gap-1 text-yellow-500 hover:text-yellow-600"
                    onClick={() =>handleLike(item._id)}  >
                      <AiOutlineLike style={{ fontSize: "30px" }} />
                      <span>Like {item.likes ? item.likes.length : 0}</span>
                    </button>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <button style={{ border: "none", background: "none" }} className="flex items-center gap-1 hover:text-gray-700">
  ...
  <div style={{ marginTop: "12px" }}>
    <h6 style={{ fontWeight: "600", color: "#374151" }}>Comments:</h6>
    ...
    >
      {item.comments.map((comment, index) => (
        ...
        >
          <p style={{textAlign:"start", fontSize: "14px", fontWeight: "bold", color: "#333" }}>
            {comment.userName}: <span style={{ color: "#666" }}>{comment.text}</span>
          </p>
        </div>
      ))}
    </div>
